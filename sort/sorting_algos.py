...
def quick_sort(arr, left, right): # 퀵정렬
    ...
    if left < right: # 정렬할 원소가 2개 이상인 경우
        q = partition(arr, left, right) # 분할할 위치
        quick_sort(arr, left, q - 1) # 왼쪽 부분 배열 재귀 정렬
        quick_sort(arr, q + 1, right) # 오른쪽 부분 배열 재귀 정렬
    return arr 
...
def radix_sort(arr, bucket_size=10): # 기수정렬
    ...
    while max_val // factor > 0: # 최대값의 모든 자리수에 대해 반복 
        ...
        for i in range(n): # 배열의 모든 원소에 대해
            ...
            queues[digit].append(arr[i]) 

        # 각 버킷에서 원소 꺼내어 배열에 저장하기
        idx = 0 
...
for n in sizes:
    ...
    for name, func in algorithms.items(): # 각 알고리즘에 대해
        # 느린 정렬은 대규모 입력 생략
        if name in ["삽입정렬", "선택정렬"] and n > 1000: # 1000 초과 시 생략
